...
def get_daily_report(config, entity_info, report_date):
    ...
    if entity_type == 'Camera':
        ...
        logger.info(f">>>>>>>>>>.entity type = camera and daily csv file join in libs/reports/notifications.py\n")

    else:
        # entity == 'Area'
        logger.info(f">>>>>>>>>>>>>.entity type = area and daily csv file join in libs/reports/notifications.py\n")
    ...
    for file_path in daily_csv_file_paths:
        ...
        if not os.path.isfile(file_path):
            ...
            with open(file_path, newline='') as csvfile:
                ...
                logger.info(f">>>>>>>>>>>>daily csv file reader open in libs/reports/notifications.py")
                for row in reader:
