...
//
var BucketName = 'smart-social-distancing';
    ...
    if (err) {
      ...
      var htmlTemplate = [
        ...
        '<ul>',
          getHtml(files),
        '</ul>',
        '<div>Real-time video verification is here</div>',
        '<div>',
            '<a href="../templates/video_streaming.html">',
                '<button>video_feed</button>',
            '</a>',
        '</div>',
      ]
...
function viewcsv(filename) {
  ...
  s3.listObjects({Prefix: csvfileKey}, function(err, data) {
    ...
    var photos = data.Contents.map(function(file) {
      var csvKey = file.Key;
      var csvUrl = bucketUrl + encodeURIComponent(csvKey) + csvKey.replace(csvfileKey, '');
      ...
      return getHtml([
        '<span>',
          '<div>',
            ...
            //csvUrl,
            '<div>',
            '<a href=../templates/getCharts.html?key='+csvKey+'>',
                '<button>',
                   csvKey.replace(csvfileKey, ''),
                '</button>',
            '</a>',
          '</div>',
        '</span>',
