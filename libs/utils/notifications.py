...
def get_violations(file_path, interval):
    ...
    with open(file_path, 'r', newline='') as csvfile:
        ...
        self.logger.info(f">>>>>>>>>>>>>reader open csv file in libs/utils/notifications.py\n")
        for row in reader:
            row_time = datetime.strptime(row['Timestamp'], "%Y-%m-%d %H:%M:%S")
            self.logger.info(f">>>>>>>>>>>>row_time = datetime.strptime in libs/utils/notifications.py\n")
...
def check_violations(entity_type, threshold, config, entity_info, interval, should_send_email, should_send_slack):
    ...
    if entity_type == 'Camera':
        ...
        self.logger.info(f">>>>>>>>>>entity type = camera in libs/utils/notifications.py\n")
    else:
        # entity_type == 'Area'
        ...
        self.logger.info(f">>>>>>>>>>entity type = area in libs/utils/notifications.py\n")
